...
import { CategoriasService } from "../../shared/services/categorias/categorias.service";
...
export class FormularioRegistrosComponent implements OnInit, OnChanges {
  ...
  buscaCategorias(): void {
    this.categoriasService.getProductData().subscribe((categorias: string[]) => {
      this.categorias = categorias;
    });
  }

  displayCategory(categoria: any): string {
    return categoria ? categoria.name : '';
  }

  selectCategoria(categoria: any){
    this.registroForm.controls["descricao"].setValue(categoria);
    this.registroForm.controls["descricao"].markAsDirty();
  }
  ...
  submitForm() {
    ...
    this.submitEventDateFormmatter.emit(
      this.extrairMesAno(this.registroForm.get("data")?.value)
    );
  }

  salvar() {
    this.registrosFinanceirosService
      .adicionarRegistro(this.retornaRegistroParaSalvar())
      .subscribe({
        next: () => {
  ...
  atualizar() {
    this.registrosFinanceirosService
      .atualizarRegistro(this.retornaRegistroParaSalvar())
      .subscribe({
        next: (response) => {
          this.registroForm.reset();
